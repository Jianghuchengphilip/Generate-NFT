...
def generate_images(edition, count, drop_dup=True):
    ...
    op_path_json = os.path.join('output', 'edition ' + str(edition), 'json')
    zfill_count = len(str(count - 1))
    if not os.path.exists(op_path):
        os.makedirs(op_path)
        os.makedirs(op_path_json)
